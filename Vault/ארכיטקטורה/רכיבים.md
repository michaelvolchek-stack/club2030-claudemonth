...
| **Sidebar** | ניווט + [[#ProjectTree]] + [[#TagManager]] + כפתור התנתקות בתחתית (form עם `action={logout}`) |
...
| רכיב | תפקיד |
|------|-------|
| **LoginForm** | Client Component בדף `/login`. טופס עברי-RTL עם `useFormState`, רכיבי shadcn; קורא ל-[[Server Actions#auth.ts — אימות משתמש\|login]]. מוצג כ-`page.tsx` (Server Component) + `LoginForm.tsx` (client form) |
