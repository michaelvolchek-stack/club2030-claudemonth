...
>
> **סוגים לא-מטופלים מתועדים בלוגים:** כל `typeWebhook` שאינו ב-`HANDLED_TYPES` מקבל `console.log` קצר (הסוג בלבד, בלי payload). מקרה מיוחד: `quotaExceeded` — נשלח בתוכנית החינמית כשמכסת ה-correspondents חורגת, **ומחליף את ההודעות האמיתיות** כך שהצ'אט הדו-כיווני מת בשקט; ה-route מדפיס עליו `console.warn` ייעודי עם `quotaData.description` כדי שהבעיה תהיה גלויה בלוגי Vercel.
